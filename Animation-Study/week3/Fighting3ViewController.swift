//
//  Fighting3ViewController.swift
//  Animation-Study
//
//  Created by 이조은 on 11/26/23.
//

import UIKit
import SnapKit

class Fighting3ViewController: UIViewController {

    //    override func loadView() {
    //        super.loadView()
    //        let view = CountdownProgressBar(frame: UIScreen.main.bounds)
    //    }
    ...
    override func viewDidLoad() {
        ...
        self.countdownProgressView.frame.size = CGSize(width: 100, height: 100)
        self.countdownProgressView.center = self.view.center
        self.view.addSubview(self.countdownProgressView)

        self.countdownProgressView.startCountDown(duration: 3)
    }
...
class CountdownProgressBar: UIView {
    ...
    let netflixImage = UIImageView(image: UIImage(named: "netflix"))
    ...
    private func loadLayers() {
        ...
        self.addSubview(netflixImage)
        netflixImage.snp.makeConstraints {
...
extension CountdownProgressBar: CAAnimationDelegate {
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        ...
        timer.invalidate()
        print("종료")
        self.netflixImage.isHidden = false
        animatePulseLayer()
    }
}
